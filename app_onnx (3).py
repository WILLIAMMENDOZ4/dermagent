...
<div class="header">
    ...
    <div>
        ...
        <p class="header-sub">
            ...
            &nbsp;
            <span class="badge badge-blue">Multilingüe</span>
        </p>
    </div>
</div>
""", unsafe_allow_html=True)
...
if not st.session_state.modal_cerrado:
    st.markdown("""
    <div style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:100; display:flex; align-items:center; justify-content:center; pointer-events:none;">
        <div style="background:#ffffff; border-radius:16px; padding:2rem; max-width:520px; width:90%; box-shadow:0 20px 60px rgba(0,0,0,0.3); pointer-events:auto;">
            <div style="display:flex; align-items:center; gap:12px; margin-bottom:1rem;">
                <div style="width:48px; height:48px; background:#0f172a; border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:24px;">🩺</div>
                <div>
                    <p style="margin:0; font-size:1.2rem; font-weight:600; color:#0f172a;">Bienvenido a DermAI</p>
                    <p style="margin:0; font-size:0.8rem; color:#64748b;">Asistente Dermatológico Inteligente</p>
                </div>
            </div>
            <p style="color:#334155; font-size:0.9rem; line-height:1.6; margin-bottom:1rem;">
                <b>DermAI</b> es un sistema de apoyo diagnóstico que combina <b>visión artificial</b> y <b>procesamiento de lenguaje natural</b> para analizar lesiones cutáneas.
            </p>
            <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; padding:1rem; margin-bottom:1rem;">
                <p style="margin:0 0 0.6rem; font-size:0.8rem; font-weight:600; color:#0f172a;">¿Cómo funciona?</p>
                <div style="display:flex; gap:8px; margin-bottom:6px; align-items:flex-start;">
                    <span style="background:#0f172a; color:white; border-radius:50%; width:20px; height:20px; display:flex; align-items:center; justify-content:center; font-size:0.7rem; font-weight:600; flex-shrink:0;">1</span>
                    <span style="font-size:0.82rem; color:#334155;">Sube una imagen de la lesión cutánea</span>
                </div>
                <div style="display:flex; gap:8px; margin-bottom:6px; align-items:flex-start;">
                    <span style="background:#0f172a; color:white; border-radius:50%; width:20px; height:20px; display:flex; align-items:center; justify-content:center; font-size:0.7rem; font-weight:600; flex-shrink:0;">2</span>
                    <span style="font-size:0.82rem; color:#334155;">El modelo IRV2 + Soft Attention clasifica entre 7 tipos de lesiones</span>
                </div>
                <div style="display:flex; gap:8px; align-items:flex-start;">
                    <span style="background:#0f172a; color:white; border-radius:50%; width:20px; height:20px; display:flex; align-items:center; justify-content:center; font-size:0.7rem; font-weight:600; flex-shrink:0;">3</span>
                    <span style="font-size:0.82rem; color:#334155;">El asistente NLP responde tus preguntas en lenguaje natural</span>
                </div>
            </div>
            <div style="background:#fef2f2; border:1px solid #fecaca; border-radius:8px; padding:0.7rem 1rem; margin-bottom:1.2rem;">
                <p style="margin:0; font-size:0.78rem; color:#b91c1c;">⚕️ <b>Aviso:</b> Esta herramienta es de uso académico. No reemplaza el diagnóstico de un dermatólogo certificado.</p>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_b:
        if st.button("✓ Entendido — Comenzar análisis", key="cerrar_modal", use_container_width=True):
            st.session_state.modal_cerrado = True
            st.rerun()
    st.stop()

if st.session_state.modal_cerrado:
    ...
    if st.session_state.resultado:
        ...
        <div class="card" style="border-left: 4px solid {color};">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    <div class="card-title">DIAGNÓSTICO SUGERIDO</div>
                    <div class="diag-name">{info['nombre']}</div>
                    <div class="diag-tipo" style="color:{color};">{info['tipo']} &nbsp;·&nbsp; {info['urgencia_icon']} Urgencia {info['urgencia']}</div>
                </div>
                <div style="background:{color_bg}; color:{color_text}; border:1px solid {color}; border-radius:8px; padding:4px 12px; font-size:0.85rem; font-weight:600;">{info['codigo_display']}</div>
            </div>
            <div class="stat-row">
                <div class="stat-box">
                    <div class="stat-val">{r['confianza']*100:.1f}%</div>
                    <div class="stat-lbl">Confianza</div>
                </div>
                <div class="stat-box">
                    <div class="stat-val">{info['tipo']}</div>
                    <div class="stat-lbl">Clasificación</div>
                </div>
                <div class="stat-box">
                    <div class="stat-val">{info['urgencia']}</div>
                    <div class="stat-lbl">Urgencia</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.expander("Ver descripción completa"):
            ...
            <div class="info-row"><span class="info-lbl">Recomendación</span><span class="info-val" style="color:{color}; font-weight:500;">{info['recomendacion']}</span></div>
            """, unsafe_allow_html=True)

        st.markdown('<div class="card-title" style="margin-top:1rem;">Distribución de probabilidades</div>', unsafe_allow_html=True)
        for cls, prob in zip(CLASES, r['probs']):
            nombre_c = INFO_CLASES[cls]['nombre'][:24]
            codigo_c = INFO_CLASES[cls]['codigo_display']
            pct = prob * 100
            bar_color = color if cls == r['clase'] else '#e2e8f0'
            fill_color = color if cls == r['clase'] else '#94a3b8'
            weight = '600' if cls == r['clase'] else '400'
            st.markdown(f"""
            <div style="margin-bottom:4px;">
                <div style="display:flex; justify-content:space-between; font-size:0.78rem;">
                    <span style="font-weight:{weight}; color:{'#ffffff' if cls==r['clase'] else '#94a3b8'};">{codigo_c} · {nombre_c}</span>
...
with col3:
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem;">
        <span class="card-title" style="margin:0;">💬 Asistente inteligente</span>
        <span class="badge badge-blue">NLP · Transformers</span>
    </div>
    """, unsafe_allow_html=True)

    chat_html = '<div class="chat-container" id="chat">'
    ...
    if st.session_state.resultado:
        ...
        if pregunta:
            ...
            with st.spinner("Procesando con NLP semántico..."):
                nlp = cargar_nlp()
    ...
    <div style="background:#f0f9ff; border:1px solid #bae6fd; border-radius:8px; padding:0.6rem 1rem; margin-top:0.8rem; font-size:0.75rem; color:#0369a1;">
        <b>Preguntas sugeridas:</b> "¿Es peligroso?", "¿Qué tratamiento hay?", "¿Cuándo debo ir al médico?", "¿Cómo me protejo del sol?"
    </div>
    """, unsafe_allow_html=True)

st.markdown("""
<div class="disclaimer">
    ⚕️ <b>Aviso médico importante:</b> DermAI es una herramienta académica de apoyo basada en inteligencia artificial (IRV2 + Soft Attention + NLP Semántico).
    <b>No constituye un diagnóstico médico definitivo</b> y no reemplaza la evaluación de un dermatólogo certificado.
    Ante cualquier lesión sospechosa, consulta siempre a un profesional de la salud.
</div>
""", unsafe_allow_html=True)
...
with st.expander("ℹ️ Acerca de DermAI — Arquitectura y lesiones detectables"):
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**Modelo de visión**\nInceptionResNetV2 + Soft Attention\nDataset: HAM10000 (10,015 imágenes)\nExportado a ONNX para producción")
    with c2:
        st.markdown("**Modelo de lenguaje**\nparaphrase-multilingual-MiniLM-L12-v2\nSentence Transformers (HuggingFace)\nComprensión semántica multilingüe")
    st.markdown("---")
    cols = st.columns(2)
