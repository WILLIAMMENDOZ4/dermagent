...
def es_imagen_dermatologica(img: Image.Image) -> tuple[bool, str]:
    """
    Heurística multifactor para detectar si una imagen es dermatológica.
    Retorna (es_valida, mensaje_rechazo).
    """
    ...
    # 1) Rechazo por cielo azul dominante
    #    (azul alto, rojo bajo, canal azul >> canal rojo)
    if b_mean > 130 and b_mean > r_mean * 1.4 and b_mean > g_mean * 1.1:
        return False, "🌤️ La imagen parece ser un paisaje o cielo. Por favor sube una **fotografía de una lesión cutánea** (lunar, mancha, peca, etc.) tomada de cerca sobre la piel."

    # 2) Rechazo por imagen mayormente verde (vegetación / naturaleza)
    if g_mean > 120 and g_mean > r_mean * 1.25 and g_mean > b_mean * 1.1:
        return False, "🌿 La imagen contiene mucho verde, parece una foto de naturaleza o exterior. Por favor sube una **imagen de piel con la lesión claramente visible**."

    # 3) Rechazo por imagen mayormente gris/blanca sin tonos piel
    #    (muy desaturada y muy clara → posible objeto, documento, etc.)
    sat = arr.std(axis=2).mean()
    brightness = arr.mean()
    if sat < 12 and brightness > 180:
        return False, "📄 La imagen parece ser un documento, fondo blanco u objeto. Por favor sube una **foto de la piel con la lesión** que deseas analizar."

    # 4) Verificación de tonos de piel (rango amplio para distintas etnias)
    #    Tono piel: R > G > B generalmente, con rango de brillo moderado
    skin_mask = (
        (R > 60) & (R < 255) &
        (G > 30) & (G < 220) &
        (B > 15) & (B < 200) &
        (R > G) & (R > B) &          # rojo dominante
        ((R - G) > 8) &               # diferencia mínima R-G
        ((R - B) > 15)                # diferencia mínima R-B
    )
    pct_piel = skin_mask.mean()

    if pct_piel < 0.08:
        return False, (
            "🔍 No se detectaron tonos de piel suficientes en la imagen. "
            "Asegúrate de que la foto muestre claramente la **lesión sobre la piel**, "
            "bien iluminada y de cerca (máximo 20–30 cm de distancia)."
        )

    # 5) Rechazo por imagen demasiado oscura (foto nocturna, subexpuesta)
    if brightness < 35:
        return False, "🌑 La imagen está muy oscura. Por favor toma la foto con **buena iluminación** y sube de nuevo."

    # 6) Rechazo por imagen demasiado clara / sobreexpuesta
    if brightness > 240 and sat < 10:
        return False, "☀️ La imagen está sobreexpuesta o es casi completamente blanca. Por favor sube una foto con **iluminación normal**."
...
if not st.session_state.modal_cerrado:
    st.markdown("""
    <div style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.75); z-index:9998; display:flex; align-items:center; justify-content:center; pointer-events:none;">
        <div style="background:#ffffff; border-radius:16px; padding:2rem 2.5rem; max-width:520px; width:90%; pointer-events:none;">
...
with col1:
    ...
    if archivo:
        ...
        if st.button("🔬 Analizar lesión"):
            # ── Validación previa ────────────────────
            es_valida, msg_rechazo = es_imagen_dermatologica(img)
            if not es_valida:
                st.markdown(f"""
                <div style="background:#fef3c7; border:1.5px solid #f59e0b; border-radius:12px;
                            padding:1rem 1.2rem; margin-top:0.5rem;">
                    <div style="font-size:1.1rem; font-weight:700; color:#92400e; margin-bottom:0.4rem;">
                        ⚠️ Imagen no válida para análisis
                    </div>
                    <div style="font-size:0.88rem; color:#78350f; line-height:1.6;">
                        {msg_rechazo}
                    </div>
                    <div style="margin-top:0.8rem; background:#fffbeb; border-radius:8px;
                                padding:0.6rem 0.9rem; font-size:0.8rem; color:#92400e;">
                        <b>💡 Consejos para una buena foto:</b><br>
                        • Fotografía directamente sobre la piel, a 10–20 cm de distancia<br>
                        • Usa luz natural o buena iluminación artificial<br>
                        • Centra la lesión en el encuadre<br>
                        • Evita sombras, reflejos o fondos distractores
                    </div>
                </div>
                """, unsafe_allow_html=True)
            else:
